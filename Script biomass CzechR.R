...
#Campaign1
cam1 <- lapply(excel_sheets("/length-biomass datafiles Czech/Czech_1st campaign.xlsx"), function(x)
  read_excel("/length-biomass datafiles Czech/Czech_1st campaign.xlsx", sheet = x, range = cell_cols("A:Y")))
...
#Campaign2
cam2 <- lapply(excel_sheets("/length-biomass datafiles Czech/Czech_2nd campaign.xlsx"), function(x)
  read_excel("/length-biomass datafiles Czech/Czech_2nd campaign.xlsx", sheet = x,range = cell_cols("A:Y")))
...
#Campaign3
cam3 <- lapply(excel_sheets("/length-biomass datafiles Czech/Czech_3rd campaign.xlsx"), function(x)
  read_excel("/length-biomass datafiles Czech/Czech_3rd campaign.xlsx", sheet = x,range = cell_cols("A:Y")))
...
#Campaign4
cam4 <- lapply(excel_sheets("/length-biomass datafiles Czech/Czech_4th campaign.xlsx"), function(x)
  read_excel("/length-biomass datafiles Czech/Czech_4th campaign.xlsx", sheet = x,range = cell_cols("A:Y")))
...
#Campaign5
cam5 <- lapply(excel_sheets("/length-biomass datafiles Czech/Czech_5th campaign.xlsx"), function(x)
  read_excel("/length-biomass datafiles Czech/Czech_5th campaign.xlsx", sheet = x,range = cell_cols("A:Y")))
...
#Campaign6
cam6 <- lapply(excel_sheets("/length-biomass datafiles Czech/Czech_6th campaign.xlsx"), function(x)
  read_excel("/length-biomass datafiles Czech/Czech_6th campaign.xlsx", sheet = x,range = cell_cols("A:Y")))
...
write.csv(data, "Campaign data incl taxonomic information Czech.csv")

#### 2: add taxonomic information for all based on genus ####

data <- read.csv("Campaign data incl taxonomic information Czech.csv")
...
# take sample code from image name
d$site <- sub(".*VEL", "", d$image_ID)
d$site <- substr(d$site, start = 1, stop = 2)
d$site <- gsub(" ", "", paste("VEL",d$site))
d$site

d <- d[d$country != "NA",] # remove missing data

write.csv(d, "Dryver Czech length and biomass data all campaigns.csv")
