...
#Campaign1
cam1 <- lapply(excel_sheets("/length-biomass datafiles Croatia/Croatia_1st campaign.xlsx"), function(x)
  read_excel("/length-biomass datafiles Croatia/Croatia_1st campaign.xlsx", sheet = x, range = cell_cols("A:Y")))
...
#Campaign2
cam2 <- lapply(excel_sheets("/length-biomass datafiles Croatia/Croatia_2nd campaign.xlsx"), function(x)
  read_excel("/length-biomass datafiles Croatia/Croatia_2nd campaign.xlsx", sheet = x,range = cell_cols("A:Y")))
...
#Campaign3
cam3 <- lapply(excel_sheets("/length-biomass datafiles Croatia/Croatia_3rd campaign.xlsx"), function(x)
  read_excel("/length-biomass datafiles Croatia/Croatia_3rd campaign.xlsx", sheet = x,range = cell_cols("A:Y")))
...
#Campaign4
cam4 <- lapply(excel_sheets("/length-biomass datafiles Croatia/Croatia_4th campaign.xlsx"), function(x)
  read_excel("/length-biomass datafiles Croatia/Croatia_4th campaign.xlsx", sheet = x,range = cell_cols("A:Y")))
...
#Campaign5
cam5 <- lapply(excel_sheets("/length-biomass datafiles Croatia/Croatia_5th campaign.xlsx"), function(x)
  read_excel("/length-biomass datafiles Croatia/Croatia_5th campaign.xlsx", sheet = x,range = cell_cols("A:Y")))
...
#Campaign6
cam6 <- lapply(excel_sheets("/length-biomass datafiles Croatia/Croatia_6th campaign.xlsx"), function(x)
  read_excel("/length-biomass datafiles Croatia/Croatia_6th campaign.xlsx", sheet = x,range = cell_cols("A:Y")))
...
cam <- cam[cam$genus != "Genus / Higher taxonomic group",]
str(cam)

levels(as.factor(cam$genus))
...
write.csv(data, "Campaign data incl taxonomic information Croatia.csv")

#### 2: add taxonomic information for all based on genus ####

data <- read.csv("Campaign data incl taxonomic information Croatia.csv")
...
para <- para %>% select(class, order, family, genus, a, b) %>% 
  mutate(genus =genus) %>%
  as.data.frame()
para$class <- as.factor(para$class)
...
d$site <- sub(".*BUT", "", d$image_ID)
d$site <- substr(d$site, start = 1, stop = 2)
d$site <- gsub(" ", "", paste("BUT",d$site))
d$site

d <- d[d$country != "NA",] # remove missing data


write.csv(d, "Dryver Croatia length and biomass data all campaigns.csv")
